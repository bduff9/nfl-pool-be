...
export const updateSurvivorMV = async (week: number): Promise<void> => {
	try {
		...
		select 0 as \`Rank\`, false as Tied, U.UserID, U.UserName, case when U.UserTeamName is null or U.UserTeamName = '' then concat(U.UserFirstName, '\\'s team') else U.UserTeamName end as TeamName, (select count(*) from SurvivorPicks SP where SP.UserID = S.UserID and SP.SurvivorPickDeleted is null and SP.SurvivorPickWeek <= ${week}) as WeeksAlive, case when S.SurvivorPickDeleted is not null then false when S.TeamID is null then false when S.TeamID = G.WinnerTeamID or G.WinnerTeamID is null then true else false end as IsAliveOverall, case when S.SurvivorPickDeleted is not null then null when S.TeamID is null then 'Dead' when S.TeamID = G.WinnerTeamID then 'Alive' when G.WinnerTeamID is null then 'Waiting' end as CurrentStatus, (select TeamID from SurvivorPicks SP2 where SP2.UserID = S.UserID and SP2.SurvivorPickWeek <= ${week} and SP2.SurvivorPickDeleted is null order by SP2.SurvivorPickWeek desc limit 1) as LastPick from SurvivorPicks S join Games G on S.GameID = G.GameID join Users U on S.UserID = U.UserID where S.SurvivorPickWeek = ${week} order by IsAliveOverall desc, WeeksAlive desc`);
...
export const updateWeeklyMV = async (week: number): Promise<void> => {
	try {
		await WeeklyMV.query(
			`lock tables WeeklyMV write, WeeklyMV as W write, WeeklyMV as W2 write, SystemValues read, Picks as P read, Games as G read, Games as G2 read, Games as G3 read, Games as G4 read, Users as U read, Tiebreakers as T read`,
		);
